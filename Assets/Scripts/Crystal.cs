...
public class SimpleGemsAnim : MonoBehaviour
{
    ...
    // --- IDE KERÜLT A BEILLESZTETT ÚJ KÓDRÉSZ ---
    private void OnTriggerEnter2D(Collider2D collision)
    {
        // Ellenőrizzük, hogy a játékos ért-e hozzá
        if (collision.CompareTag("Player"))
        {
            // Megkeressük a BossSpawner scriptet a pályán
            BossSpawner boss = Object.FindFirstObjectByType<BossSpawner>();

            if (boss != null)
            {
                // Ha megvan a Boss, megsebezzük 1 egységgel
                boss.TakeDamage(1);
            }

            // Eltüntetjük a drágakövet a pályáról
            Destroy(gameObject);
        }
    }
}